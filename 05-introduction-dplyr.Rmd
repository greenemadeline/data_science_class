...
CO2_tibble %>%
  filter(Type == "Mississippi", Treatment == "chilled") %>%
  select(Plant, Type, Treatment) -> new_co2
...
```{r}
CO2_tibble %>%
  mutate(mean_uptake = mean(uptake)) #newname=oldname

CO2_tibble %>%
  summarize(mean_uptake = mean(uptake)) #summaize the same function as in mutate to get one answer
```
...
```{r}
quebec %>%
  summarise(median = median(uptake))
```
...
quebec %>%
 filter(conc == 500) %>%
  summarize(median = median(uptake))
```
...
```{r}
CO2_tibble %>%
  filter(Treatment == "nonchilled", conc == 1000) %>% 
  group_by(Type) %>% #need to group it by the type since comparing different types of plant
  summarize(mean = mean(uptake)) #therefore, quebec has the higher mean uptake
```
...
### q14 code way 1
CO2_tibble %>%
  dplyr:: filter (Treatment == "chilled") -> Chilled_tibble #you have to make a new tibble where only chilled is showing


ggplot(Chilled_tibble, aes(x = conc, y = uptake, group = Plant, color = Treatment)) + 
  geom_line() + 
  geom_point() + 
  facet_grid(~Type) + 
  labs(x = "Concentration", y = "Uptake")
...
```{r}
CO2_tibble %>%
  filter(conc == 175) %>% #choosing only equal to 175
...
new_levels <- c("Mississippi", "Quebec") #reordering on graph
new_labels <- c("Nonchilled", "Chilled") # renaming so capital 

CO2_tibble %>%
  mutate(species_factor_custom_levels = factor(Type, levels = new_levels)) %>% 
  mutate(species_factor_custom_labels = factor(Treatment, labels = new_labels)) %>% #mutating so new labels appears
  group_by(species_factor_custom_labels, species_factor_custom_levels, conc) %>% 
  summarize(mean_uptake = mean(uptake)) %>%
  ggplot(aes(x = factor(conc), y = mean_uptake, fill = species_factor_custom_labels)) + # factor (conc) 
  geom_bar(stat="identity") +
  facet_grid(species_factor_custom_levels ~ species_factor_custom_labels) + 
  labs(x = "CO2 Concentration", y = "Mean uptake") + 
  theme_dark () +  
  theme(legend.position = "NONE") +  #dont want legend
  scale_fill_brewer()
